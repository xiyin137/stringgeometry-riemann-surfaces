...
instance skyscraperModule_isCoherent (p : C.PointType) :
  ...
  locallyFinitelyGenerated := fun i => by
    ...
    change Module.Finite ↑(C.toScheme.ringCatSheaf.val.obj U)
    ...
    · -- p ∈ U: sections are κ(p), finitely generated by {fromKappa(1)}
      ...
      have hUtop : U.unop ≤ (⊤ : TopologicalSpace.Opens C.toScheme.carrier) := by
        intro x hx
        trivial
      ...
      let r : ↑(C.toScheme.presheaf.obj U) :=
        (C.toScheme.presheaf.map (homOfLE hUtop).op).hom globalSec
      refine ⟨r, ?_⟩
      ...
      change (evalAtPoint p U.unop hp r) * 1 = v
      -- r = res_{U≤⊤}(globalSec), so evaluate via restriction-compatibility.
      have hEval : evalAtPoint p U.unop hp r = v := by
        change evalAtPoint p U.unop hp
          ((C.toScheme.presheaf.map (homOfLE hUtop).op).hom globalSec) = v
        rw [SkyscraperConstruction.evalAtPoint_comp_restriction
          p U.unop ⊤ hp (Set.mem_univ _) hUtop globalSec]
