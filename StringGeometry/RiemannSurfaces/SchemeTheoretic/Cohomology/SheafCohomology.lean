...
private theorem h0_compat {X : Scheme} (F : OModule X) (𝒰 : OpenCover X)
  ...
  have hleft :
      F.val.map (homOfLE (inf_le_left : V i ⊓ V j ≤ V i)).op (c (fun _ => i)) =
        F.val.map (homOfLE h₁).op (c (fun _ => i)) := by
    exact OModule.map_eq F _ _ _
  have hright :
      F.val.map (homOfLE (inf_le_right : V i ⊓ V j ≤ V j)).op (c (fun _ => j)) =
        F.val.map (homOfLE h₀).op (c (fun _ => j)) := by
    exact OModule.map_eq F _ _ _
  exact hleft.trans (heq_F.trans hright.symm)
...
private theorem h0_surj_aux {X : Scheme} (F : OModule X) (𝒰 : OpenCover X)
  ...
  let V : 𝒰.I → Opens X.carrier := fun i => 𝒰.intersection (fun (_ : Fin 1) => i)
  let sf : ∀ i : 𝒰.I, F.val.obj (Opposite.op (V i)) := fun i => c (fun _ => i)
  let hTop : ∀ i : 𝒰.I, V i ≤ (⊤ : Opens X.carrier) := by
    intro i x hx
    trivial
  have compat := h0_compat F 𝒰 c hcoc
  obtain ⟨s_top, hs⟩ := OModule.glue_sections_top F V hVeq hTop sf compat
...
theorem h0_eq_globalSections (F : OModule C.toScheme) :
  ...
  let hTopV : ∀ i : 𝒰.I, V i ≤ (⊤ : Opens C.toScheme.carrier) := by
    intro i x hx
    trivial
  let hTopσ : ∀ σ : Fin 1 → 𝒰.I, 𝒰.intersection σ ≤ (⊤ : Opens C.toScheme.carrier) := by
    intro σ x hx
    trivial
  have hVeq : ⨆ i, V i = ⊤ := by
  ...
  let φ : GlobalSectionsType C.toAlgebraicCurve F → CechCocycles F 𝒰 0 :=
    fun s => ⟨fun σ => F.val.map (homOfLE (hTopσ σ)).op s, by
      funext τ
      change cechDifferential F 𝒰 0 (fun σ => F.val.map (homOfLE (hTopσ σ)).op s) τ = 0
      ...
      have := fun j => OModule.map_comp_apply F
        (intersection_face_le 𝒰 τ j) (hTopσ (faceMap j τ)) s
  ...
  have hφ_inj : Function.Injective φ := by
    ...
    exact TopCat.Sheaf.eq_of_locally_eq'
      ...
      V ⊤ (fun i => homOfLE (hTopV i)) (le_of_eq hVeq.symm) s t
  ...
  have hφ_surj : Function.Surjective φ := by
    ...
    refine ⟨s, Subtype.ext (funext (fun σ => ?_))⟩
    have hmap :
        F.val.map (homOfLE (hTopσ σ)).op s =
          F.val.map (homOfLE
            (show 𝒰.intersection σ ≤ (⊤ : Opens C.toScheme.carrier) from by
              intro x hx
              trivial)).op s := by
      exact OModule.map_eq F _ _ s
    exact hmap.trans (hs σ)
